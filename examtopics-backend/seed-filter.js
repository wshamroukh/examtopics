...
async function main() {
  ...
  let examList = JSON.parse(fs.readFileSync(EXAMS_MANIFEST, 'utf8'));
  console.log(`Found ${examList.length} exams in exams.json`);

  // Optional filter: node seed.js ai-900
  const filterArg = process.argv[2];
  if (filterArg) {
    examList = examList.filter(e => e.id === filterArg);
    if (examList.length === 0) {
      console.error(`\nNo exam found with id "${filterArg}". Check exams.json for the correct id.`);
      process.exit(1);
    }
    console.log(`Filtering to: ${filterArg}\n`);
  } else {
    console.log('');
  }
